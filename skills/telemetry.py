"""
Skill Usage Tracking System (Skill Telemetry)

Reference Hermes Agent's skill_usage.py implementation
Records skill usage, viewing, and modification events
Supports Curator's lifecycle management and intelligent merging

Uses .skill_usage.json sidecar file for tracking, avoiding pollution of user's SKILL.md content
"""
...
class SkillTelemetry:
    ...
    def get_agent_created_skills(self) -> List[SkillUsageRecord]:
        """Get agent-created skills (managed by Curator)"""
        return [
    ...
    def record_patch(self, skill_id: str) -> None:
        """
        Record skill modification event
    ...
    def create_skill_record(
        ...
        """
        Create usage record for new skill

        Args:
            skill_id: Skill ID
            created_by: Creator ("agent", "user", "bundled", "hub")
            tags: Skill tags
    ...
    def get_skill_report(self) -> List[Dict[str, Any]]:
        """
        Get usage report for all skills

        Returns:
            List of skill reports containing usage statistics and state info
        """
        reports = []
    ...
    def archive_skill(self, skill_id: str, target_dir: Optional[Path] = None) -> bool:
        ...
        Move skill to .archive directory and set state to archived

        Args:
            skill_id: Skill ID
            target_dir: Archive target directory (default: skills/.archive/)
    ...
    def delete_skill_record(self, skill_id: str) -> bool:
        """
        Delete skill usage record

        Note: This will not delete actual skill files, only the tracking record
...
def get_skill_telemetry() -> SkillTelemetry:
    """Get global skill tracker instance"""
