...
Reference Hermes Agent's Curator umbrella building strategy
Automatically identify similar skills and merge into umbrella skills

Merge strategy:
1. Identify prefix clusters
2. Merge into existing umbrella skills
3. Create new umbrella skills
4. Downgrade to references/templates/scripts
"""
...
class SkillMerger:
    ...
    def identify_umbrella_skill(self, cluster: SkillCluster) -> Optional[SkillInfo]:
        """
        Identify umbrella skill

        Umbrella skill characteristics:
        - More generic name
        - Description covers multiple sub-skills
        - Already a merge target for multiple skills

        Args:
            cluster: Skill cluster

        Returns:
            Umbrella skill (if any)
        """
    ...
    def find_similar_skills(self, skill: SkillInfo, candidates: List[SkillInfo]) -> List[Tuple[SkillInfo, float]]:
        """
        Find skills similar to the specified skill
        ...
        Returns:
            List of similar skills, sorted by similarity descending
        """
        similarities = []
    ...
    def merge_skills(
        ...
        Merge source skill into target umbrella skill:
        1. Add section to target skill's SKILL.md
        2. Move source skill to references/ subdirectory

        Args:
            source: Source skill
            target: Target umbrella skill
            target_dir: Skills directory
...
"""
    ...
    def create_umbrella_skill(
        ...
        """
        Create new umbrella skill

        Args:
            cluster: Skill cluster
            skills_dir: Skills root directory

        Returns:
            Created umbrella skill (if any)
        """
        try:
