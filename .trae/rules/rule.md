...
### 6. 复杂任务先规划到 TODO ⚠️ **重要**

**必须遵循**：做复杂事情（多步骤、跨模块、需多个文件修改）时，必须先将任务分解并记录到 `TODO.md` 中，然后再开始实现。

**流程**：
1. **理解需求** → 分解为子任务
2. **记录 TODO** → 使用 TodoWrite 工具创建列表
3. **查看历史** → 检查 `TODO.md` 中是否有相关内容可以复用
4. **综合规划** → 制定完整的实现路径
5. **按步执行** → 逐个完成任务，更新 TODO 状态

**必须使用 TodoWrite**：
- 当任务需要 ≥ 3 个工具调用时
- 当任务涉及 ≥ 2 个模块/文件时
- 当用户说"记录 todo"或"一步步来"时

**示例**：
```
- [ ] 实现技能管理 CLI 命令
  - [ ] CLI skills install 命令
  - [ ] CLI skills list 命令
  - [ ] CLI skills uninstall 命令
  - [ ] 更新 README 文档
```
