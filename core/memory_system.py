"""Enhanced Memory System - 增强的多层记忆系统

整合会话记忆、持久记忆、情景记忆，构建完整的记忆架构。

参考 Hermes Agent 的多层记忆设计：
1. Working Memory - 工作记忆（当前上下文）
2. Session Memory - 会话记忆（当前会话历史）
3. Persistent Memory - 持久记忆（跨会话知识）
4. Episodic Memory - 情景记忆（重要事件记录）
...
class EnhancedMemorySystem:
    ...
    def __init__(
        ...
        Args:
            ...
            enable_auto_curation: 启用自动整理
            max_working_items: 工作记忆最大条目数
            max_session_items: 会话记忆最大条目数
        """
