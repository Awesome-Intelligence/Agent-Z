...
# -*- coding: utf-8 -*-
"""
Tests for the API server adapter.
"""

# 🏃 Execution - 🛠️ ToolExec - API Server Tests
...
class TestAPIServerAdapter:
    """Tests for APIServerAdapter class."""

    def test_init_defaults(self):
        adapter = APIServerAdapter()
        assert adapter._host == "127.0.0.1"
        assert adapter._port == 8001
    ...
    def test_parse_cors_origins_empty(self):
        result = APIServerAdapter._parse_cors_origins("")
        assert result == ()

    def test_parse_cors_origins_single(self):
        result = APIServerAdapter._parse_cors_origins("https://example.com")
        assert result == ("https://example.com",)

    def test_parse_cors_origins_multiple(self):
        result = APIServerAdapter._parse_cors_origins("https://a.com, https://b.com")
    ...
    def test_origin_allowed_with_asterisk(self):
        adapter = APIServerAdapter({"extra": {"cors_origins": "*"}})
        assert adapter._origin_allowed("https://any.com") is True

    def test_origin_allowed_specific(self):
        adapter = APIServerAdapter({"extra": {"cors_origins": "https://allowed.com"}})
    ...
    def test_check_auth_invalid_token(self):
        adapter = APIServerAdapter({"extra": {"key": "correct-key"}})
        request = MagicMock()
